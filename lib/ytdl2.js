...
const fetchBuffer = async (url, options) => { try { options ? options : {}; const res = await axios({ method: "GET", url, headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.70 Safari/537.36", 'DNT': 1, 'Upgrade-Insecure-Request': 1 }, ...options, responseType: 'arraybuffer' }); return res.data } catch (err) { return err } }
     ...
     */
    const WriteTags = async (filePath, Metadata) => { NodeID3.write({ title: Metadata.Title, artist: Metadata.Artist, originalArtist: Metadata.Artist, image: { mime: 'jpeg', type: { id: 3, name: 'front cover' }, imageBuffer: (await fetchBuffer(Metadata.Image)).buffer, description: `Cover of ${Metadata.Title}` }, album: Metadata.Album, year: Metadata.Year || '' }, filePath) }
     ...
     * @returns 
     */
    const search = async (query, options = {}) => { const search = await yts.search({ query, hl: 'id', gl: 'ID', ...options }); return search.videos }
    ...
    const downloadMusic = async (query) => {
        try {
            const getTrack = Array.isArray(query) ? query : await searchTrack(query);
            ...
            await WriteTags(file, { Title: search.title, Artist: search.artist, Image: search.image, Album: search.album, Year: videoInfo.videoDetails.publishDate.split('-')[0] });
     ...
     * @returns
     */
    const mp4 = async (query, quality = 480) => {
        try {
     ...
     * @returns 
     */
    const mp3 = async (url, metadata = {}, autoWriteTags = false) => {
        try {
            ...
            const file = await new Promise((resolve) => { ffmpeg(stream).audioFrequency(44100).audioChannels(2).audioBitrate(128).audioCodec('libmp3lame').audioQuality(5).toFormat('mp3').save(songPath).on('end', () => { resolve(songPath) }) })
            if (Object.keys(metadata).length !== 0) { await WriteTags(file, metadata) }
            if (autoWriteTags) { await WriteTags(file, { Title: videoDetails.title, Album: videoDetails.author.name, Year: videoDetails.publishDate.split('-')[0], Image: videoDetails.thumbnails.slice(-1)[0].url }) }
            return {
                meta: {
                ...
                }, path: file, size: fs.statSync(songPath).size
            }
        } catch (error) { throw error }
}
export default { fetchBuffer, mp3, mp4, downloadMusic }
