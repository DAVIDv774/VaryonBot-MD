...
function makeInMemoryStore() {
    ...
    async function fetchImageUrl(jid, profilePictureUrl) {
        ...
        if (!(jid in chats)) return chats[jid] = { id: jid }
        if (!chats[jid].imgUrl) {
            const url = await profilePictureUrl(jid, 'image').catch(() => './multimedia/imagenes/avatar_contact.png')
    ...
    function bind(ev, opts = { groupMetadata: () => null }) {
        ...
        /*ev.on('group-participants.update', async function store(groupParticipantsUpdate) {
            ...
            chats.isChats = true
            // @ts-ignore
            const groupMetadata = await opts.groupMetadata(id).catch(_ => null)
        ...
        /*ev.on('group-participants.update', async function store(groupParticipantsUpdate) {
            ...
            if (!(id in chats) || !chats[id]?.metadata) {
              const metadata = await fetchGroupMetadata(id, opts.groupMetadata);
              Object.assign(chats[id], { metadata });
            }
            
            const metadata = chats[id]?.metadata;
            if (!metadata) { console.log(`Try to update group ${id} but metadata not found in 'group-participants.update'`); return }
        ...
        /*ev.on('group-participants.update', async function store(groupParticipantsUpdate) {
            const id = groupParticipantsUpdate.id?.decodeJid?.();
            if (!id || !isJidGroup(id)) return;
            if (!(id in chats) || !chats[id]?.metadata) { const metadata = await fetchGroupMetadata(id, opts.groupMetadata); Object.assign(chats[id], { metadata })}
        ...
        /*ev.on('groups.update', async function groupUpdatePushToDb(groupsUpdates) {
            try {
                for (const update of groupsUpdates) {
                    const id = this.decodeJid(update.id)
                    if (!id || id === 'status@broadcast') continue
                    const isGroup = id.endsWith('@g.us')
                    if (!isGroup) continue
                    let chats = this.chats[id]
                    if (!chats) chats = this.chats[id] = { id }
                    chats.isChats = true
