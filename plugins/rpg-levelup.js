...
let handler = async (m, { conn }) => {
   ...
   if (!canLevelUp(user.level, user.exp, global.multiplier)) { let { min, xp, max } = xpRange(user.level, global.multiplier); m.reply(`*『 TU NIVEL ACTUAL 』*\n\n▢ Nombre : *${name}*\n▢ Nivel : *${user.level}*\n▢ XP : *${user.exp - min}/${xp}*\n▢ Rango : *${user.role}*\n\nTe falta *${max - user.exp}* de *XP* para subir de nivel`)}
      
   let before = user.level * 1
   while (canLevelUp(user.level, user.exp, global.multiplier)) user.level++
   if (before !== user.level) { user.role = global.rpg.role(user.level).name
