...
- Tienes  *60s*
Confirme que desea realizar la transferencia repondiendo con un 'si'. Si no esta deacuerdo, puede responder con un 'no' para cancelar esta acción`.trim()
...
handler.before = async m => { if (m.isBaileys) return
   ...
   if (/no?/g.test(m.text.toLowerCase())) {
      ...
      return m.reply('ransferencia Cancelada ✓')
   }
...
handler.help = ['transferir'].map(v => v + ' [tipo] [monto] [@tag]')
handler.tags = ['econ']
handler.command = ['payxp', 'paydi', 'transfer', 'darxp', 'transferir',]
