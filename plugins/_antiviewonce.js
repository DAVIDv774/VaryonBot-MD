...
export async function before(m) {
	...
	if (m.mtype == 'viewOnceMessage') { 
		...
		for await (const chunk of media) { buffer = Buffer.concat([buffer, chunk])} 
			if (/video/.test(type)) { return this.sendFile(m.chat, buffer, 'error.mp4', `${msg[type].caption}\n\n*Mensaje multimedia para verse una sola vez, recuperado ✓*\n`, m) } else if (/image/.test(type)) { return this.sendFile(m.chat, buffer, 'error.jpg', `${msg[type].caption}\n\n*Mensaje multimedia para verse una sola vez, recuperado ✓*\n`, m)}}
}
