...
global.menu_hit = []
let handler = async (m, { conn, usedPrefix: _p, __dirname, isPrems, isROwner, isOwner, isModr }) => {
  try {
    let sabiasque = sabiasq[Math.floor(Math.random() * sabiasq.length)]
    let _package = JSON.parse(await promises.readFile(join(__dirname, '../package.json')).catch(_ => ({}))) || {}
    let { exp, level } = global.db.data.users[m.sender]
    let prem = isPrems ? 'Si' : 'No'
    let role = isROwner ? '(Creador)' : isOwner ? '(Dueño)' : isModr ? '(moderador)' : global.db.data.users[m.sender].role
    ...
    let settingstatus = 0; if (new Date() * 1 - settingstatus > 1000) { await conn.query({ tag: 'iq', attrs: { to: '@s.whatsapp.net', type: 'set', xmlns: 'status', }, content: [{ tag: 'status', attrs: {}, content: Buffer.from(`『Ѵᴀʀʏᴏɴ/𝐁ᴏᴛ』Tiempo activo : ` + uptime, 'utf-8') }] }).catch((_) => _); settingstatus = new Date() * 1; }
  } catch (e) { conn.reply(m.chat, '*『 Ocurrio un error en el menú 』*', m); throw e }
}
