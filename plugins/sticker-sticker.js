...
let handler = async (m, { conn, args, usedPrefix, command, groupMetadata }) => {
  ...
  try {
    ...
    if (/webp|image|video/g.test(mime)) {
      ...
      let out
      try { stiker = await sticker(img, false, ``, `${sname ? sname : Nomowner} ● ${author}`) } catch (e) { console.error(e) } finally {
        if (!stiker) {
          if (/webp/g.test(mime)) out = await webp2png(img) 
          else if (/image/g.test(mime)) out = await uploadImage(img)
          else if (/video/g.test(mime)) out = await uploadFile(img)
          if (typeof out !== 'string') out = await uploadImage(img)
          stiker = await sticker(false, out, ``, `${sname ? sname : Nomowner} ● ${author}`)
        }
      }
    } else if (args[0]) { if (isUrl(args[0])) stiker = await sticker(false, args[0], ``, `${sname ? sname : Nomowner} ● ${author}`)
      else return m.reply('¡Url inválido! :v')} } catch (e) { console.error(e)
        if (!stiker) stiker = e } finally { if (stiker) try { conn.sendMessage(m.chat, { sticker: fs.readFileSync(stiker), mentions: [m.sender]})} catch { conn.sendFile(m.chat, stiker, 'sticker.webp', '', m); if (global.db.data.chats[m.chat].cmdRpg && m.isGroup) { m.coin = true } else {/*FUAP*/} }
        else m.reply('')}
}
