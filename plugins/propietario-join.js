let handler = async (m, { conn, text, usedPrefix, command, args, participants, isOwner }) => {
   ...
   if (!args[0]) m.reply(`Envie el link del Grupo\n\n Ejemplo:\n *${usedPrefix + command}* <linkwa> <dias>\n\n_el número son los días que el bot estará en el grupo_`)
   if (!code) m.reply(`Link inválido`)
   if (!args[1]) m.reply(`Falta el número de días\n\n Ejemplo:\n *${usedPrefix + command}* <linkwa> 2`)
   if (isNaN(args[1])) m.reply(`Solo número, que representa los días que el bot estará en el grupo!`)

   let owbot = global.owner[1]; m.reply(`😎 Espere 3 segundos, me uniré al grupo`); await delay(3000)
   try {
...
fui invitado por *${m.name}*`, m, { mentions: d }).then(async () => {await delay(7000)}).then(async () => {
      ...
      if (!e.length) await conn.reply(global.owner[1]+'@s.whatsapp.net', `≡ *INVITACIÓN A GRUPO*\n\n@${m.sender.split('@')[0]} ha invitado a *${conn.user.name}* al grupo\n\n*${await conn.getName(res)}*\n\n*ID* : ${res}\n\n📌 Enlace : ${args[0]}\n\nEl bot saldrá automáticamente después de\n\n ${msToDate(db.data.chats[res].expired - now)}`, null, {
         ...
         await conn.sendButton(res, mes, fgig, null, [[`✆ Owner`, `${usedPrefix}fgowner`], [`⦙☰ Menu`, `${usedPrefix}help`]], m, { mentions: d })})} catch (e) { conn.reply(global.owner[1]+'@s.whatsapp.net', e)
         m.reply(`Lo siento, el bot no puede unirse a grupos`) }
}
...
export default handler
   ...
   function msToDate(ms) {
      ...
      return [d,
         ...
         ' *Minutos*\n ',
         s,
         ' *Segundos* '].map(v => v.toString().padStart(2, 0)).join('')
   }